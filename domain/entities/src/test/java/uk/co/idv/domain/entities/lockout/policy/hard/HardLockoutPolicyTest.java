...
class HardLockoutPolicyTest {
    ...
    @Test
    void shouldReturnHardLockoutType() {
        assertThat(policy.getLockoutType()).isEqualTo(HardLockoutStateCalculator.TYPE);
    }

    @Test
    void shouldReturnHardStateCalculator() {
        assertThat(policy.getStateCalculator()).isInstanceOf(HardLockoutStateCalculator.class);
    }

    @Test
    void shouldSetMaxNumberOfAttemptsOnStateCalculator() {
        final HardLockoutStateCalculator calculator = (HardLockoutStateCalculator) policy.getStateCalculator();
